...
分數代表同一次分析中的相對閱讀優先度。分數不代表跨領域或跨清單通用的學術品質。
...
需求：Python 3.11 以上、Docker、Docker Compose。
...
開啟 <http://localhost:8501>。GROBID 首次啟動可能需要一至兩分鐘。

API keys 均為選填。第一次執行時，先從 `.env.example` 建立自己的 `.env`，再把申請到的資料填入 `.env`。未設定 Key 時，系統會嘗試使用各服務的公開存取額度。

### Windows 一鍵啟動

執行 `start-nextread.cmd`。啟動器會檢查 Docker Desktop、啟動 GROBID、重新啟動本專案既有的 Streamlit process、等待服務完成，並在預設瀏覽器開啟應用程式。每次執行一鍵啟動都會載入目前磁碟上的完整程式版本。
...
這組 `30 / 25 / 25 / 10 / 6 / 4` 權重是 literature-informed project baseline。文獻支持各類訊號的選擇與部分子權重。完整六維比例尚未經本專案人工標記資料集驗證。
...
CSV 不應放在專案根目錄。它不會上傳至 GitHub；每位使用者需自行下載，再透過「進階設定 → 手動更新 SJR 資料」安裝，或手動放到上述路徑。若檔名、年份或位置不同，請在 `.env` 中設定 `SJR_DATA_PATH`。
